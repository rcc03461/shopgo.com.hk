...
目標行為：**`shopgo.hk/login` 登入成功後，導向 `https://{shop_slug}.shopgo.hk/admin/dashboard`**（本機開發則為 `http://{shop_slug}.shopgo.hk:3000/admin/dashboard`），並逐步完成 README 所列租戶 Admin 頁面。
...
- **關鍵產品決策**：預設 Cookie 為 **host-only**（僅 `shopgo.hk`），**不會**自動送到 `demo.shopgo.hk`，因此若不做調整，導向子網域後台會變成「未登入」。
...
**A. Cookie `Domain=.shopgo.hk`（推薦，實作相對單純）**

- 設定 Cookie 時帶 `domain: '.shopgo.hk'`（僅在 host 為 `*.shopgo.hk` 或主站時依規則設定；**localhost 不適用**，本機需用 hosts + 自訂網域測試）。
- `SameSite=Lax`：同站頂層導向仍可帶 Cookie；若未來有跨站 POST 登入再評估 `None` + `Secure`。
- **本機**：`NUXT_PUBLIC_BASE_DOMAIN=shopgo.hk`（或 runtime `public.tenantRootDomain`），Cookie domain 與導向 URL 都讀此值；port 用 `window.location.port` 或 `NUXT_PUBLIC_DEV_PORT`。

**B. 一次性換票（ticket）**

- 主站登入成功後 `302` 到  
  `https://{slug}.shopgo.hk/admin/auth/callback?ticket=...`  
...
- `nuxt.config`：`runtimeConfig` 增加  
  - `public.tenantRootDomain`（例：`shopgo.hk`）  
...
- **Production**：`secure: true`（HTTPS）；**Dev**：維持 `secure: false`，且 Cookie domain 僅在「非 localhost」時設 `.shopgo.hk`。
...
- **Middleware**（`app/middleware/00-tenant-slug.global.ts`）：讀 `useRequestHeaders().host`，解析（實作見 `app/utils/tenantHost.ts`）：
  - `shopgo.hk` → `tenantSlug = null`（主站）
  - `demo.shopgo.hk` → `tenantSlug = 'demo'`
...
**Admin 僅在子網域可存取**：`middleware` 檢查 `tenantSlug`，若為 null 則不進 admin（`404` 或導向 `https://shopgo.hk`）。
...
- 若 Cookie 已設為 `.shopgo.hk`，子網域請求會帶上 Cookie，`/api/auth/me` 在子網域即可通過。
...
- **hosts**：已支援 `shopgo.hk`、`demo.shopgo.hk`；每新增一個測試店需一行（Windows 不支援 `*.shopgo.hk` 單行萬用字元）。
- **README**：主站 `/login` 一行已寫目標導向；完成 P0 後把「目前狀態」改為「已支援主站登入轉租戶後台殼層」。
- **Chrome DevTools**：若仍無法開 `shopgo.hk:3000`，在 **Network** 看是否 `ERR_CONNECTION_REFUSED`（綁定埠／IPv4）或 **DNS_PROBE**（hosts 未生效）；專案已設 `devServer.host: '0.0.0.0'`，需重啟 dev。Cookie 問題在 **Application → Cookies** 看 `Domain` 是否為 `.shopgo.hk`。
...
- [x] `login` 頁成功後 **`external: true`** 導向 `{slug}.shopgo.hk/admin/dashboard`
...
- **Cookie 跨子網域**：所有 `*.shopgo.hk` 皆可讀取該 Cookie；需嚴格 **HttpOnly**、避免 XSS；子網域若未來開放第三方上傳內容，要評估隔離策略（屆時可改方案 B）。
