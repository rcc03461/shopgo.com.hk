...
基於 Nuxt 的多租戶電商平台雛形，主站使用 `shopgo.com.hk`，租戶店舖使用 `shop_slug.shopgo.com.hk`。
...
### 平台主站
- `shopgo.com.hk/`
- `shopgo.com.hk/login` => login to shop_slug.shopgo.com.hk/admin/dashboard
...
- `shop_slug.shopgo.com.hk/invoices/invoice_uuid`
- `shop_slug.shopgo.com.hk/tnc`
- `shop_slug.shopgo.com.hk/p/page_slug`
- `shop_slug.shopgo.com.hk/profile`
- `shop_slug.shopgo.com.hk/profile/invoices`
- `shop_slug.shopgo.com.hk/profile/invoices/invoice_uuid`

### 租戶 admin
- `shop_slug.shopgo.com.hk/admin/dashboard`
- `shop_slug.shopgo.com.hk/admin/pages`
- `shop_slug.shopgo.com.hk/admin/categories`
- `shop_slug.shopgo.com.hk/admin/products`
- `shop_slug.shopgo.com.hk/admin/settings`
- `shop_slug.shopgo.com.hk/admin/settings/payment`
...
瀏覽器開啟：`http://shopgo.com.hk:3000`（port 依終端機輸出為準；hosts 需已指向 `127.0.0.1`）。

### 為何 `localhost` 可以、`shopgo.com.hk` 卻 `ERR_CONNECTION_REFUSED`？

在 Windows 上很常見：開發伺服器預設只綁在 **`localhost`（常為 IPv6 的 `::1`）**，而 hosts 裡的 `shopgo.com.hk` 通常解析成 **`127.0.0.1`（IPv4）**，兩者不是同一個 listening socket，就會出現連線被拒。

專案透過 **`package.json` 的 `dev` 指令**（`nuxt dev --host 0.0.0.0`）綁在 **`0.0.0.0`**，IPv4（`127.0.0.1`／`shopgo.com.hk`）與本機其他網卡都能連。`nuxt.config` 內 **`devServer.host`** 為輔助說明用；修改後請**重啟** `bun run dev`。
...
- `NUXT_PUBLIC_TENANT_ROOT_DOMAIN`：預設 `shopgo.com.hk`；本機請搭配 hosts 使用該網域（**localhost 無法使用跨子網域 Cookie**）。
...
- A 記錄：`shopgo.com.hk` -> VPS IP
- A 記錄：`*.shopgo.com.hk` -> VPS IP
- SSL 請申請同時覆蓋：`shopgo.com.hk` + `*.shopgo.com.hk`（建議 DNS Challenge）
...
NUXT_PUBLIC_TENANT_ROOT_DOMAIN=shopgo.com.hk
...
`shopgo.com.hk` 與 `*.shopgo.com.hk` 都反代到 `127.0.0.1:8555`。
